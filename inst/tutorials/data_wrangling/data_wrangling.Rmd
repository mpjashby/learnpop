---
title: "Crime analysis for problem solving: Data wrangling"
output: 
  learnr::tutorial:
    progressive: true
...
In this tutorial we will learn how to wrangle data in R using *functions* – 
specialised pieces of code that do something to the data we give it. The code to 
use a function (sometimes called *calling* the function) has two parts: the 
...
function works on, or to fine-tune the way the function works (we will see many 
...
package is include the code `library(tidyverse)` at the top of our R script. We 
will use comments (lines of code beginning with `#`) to help explain as we go.
...
Use `read_csv()` to load data from 
`https://github.com/mpjashby/crimemapping/raw/main/inst/extdata/san_francisco_robbery.csv` 
...
the data contained in it. If you do this with a data frame, R will attempt to 
print every row of data, even if this floods the console screen with data. If 
you type the name of a tibble into the console to view its contents, R will only 
print up to 10 rows, and will only print as many columns as fit on the screen. 
There are [lots of other small benefits to using tibbles](https://tibble.tidyverse.org/) 
...
You might have noticed that the code above produced a message specifying how
...
count(agg_assault_data, location_category, location_type, sort = TRUE)
```
...
character values from A to Z. We can instead sort values in descending order
...
The `agg_assault_data` object contains records of 
`r scales::comma(nrow(agg_assault_data))` serious assaults. What is the 
most-common type of location for serious assaults to occur at? Use the code box
below to experiment with the functions you've learned in this tutorial until you
get the result you need. Once you've finished, click the `Solution` button to
check the answer.

```{r user-exercise1, exercise=TRUE, exercise.lines=15}

```

```{r user-exercise1-solution}
# There are several ways you could do this, but one way is to use the `count()`
# function with the `sort` argument set to `TRUE`
agg_assault_data %>% 
  count(location_type, sort = TRUE)

# The code above produces exactly the same result as this code, which uses the
# `n()` function that returns the number of rows in each group
agg_assault_data %>% 
...
Based on the same dataset, on which day of the week were there the most serious
assaults in open public places (i.e. on streets or in other open spaces)?

```{r user-exercise2, exercise=TRUE, exercise.lines=15}

```

```{r user-exercise2-solution}
# There are several ways you could get to the answer for this question -- in
# particular, you could choose to use a `filter()` function before or after the
# `mutate()` function
agg_assault_data %>% 
  filter(location_category %in% c("open space", "street")) %>% 
